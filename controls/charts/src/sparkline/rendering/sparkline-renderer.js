define(["require", "exports", "@syncfusion/ej2-base", "../utils/helper", "../utils/helper", "@syncfusion/ej2-data"], function (require, exports, ej2_base_1, helper_1, helper_2, ej2_data_1) {
    "use strict";
    ...
    var SparklineRenderer = (function () {
        ...
        SparklineRenderer.prototype.processData = function () {
            ...
            if (ej2_base_1.isNullOrUndefined(data) || !data.length) {
                return;
            }
        ...
        SparklineRenderer.prototype.processDataManager = function () {
            ...
            var dataModule;
            var queryModule;
            if (this.sparkline.dataSource instanceof ej2_data_1.DataManager) {
                dataModule = this.sparkline.dataSource;
                queryModule = this.sparkline.query instanceof ej2_data_1.Query ? this.sparkline.query : new ej2_data_1.Query();
        ...
        SparklineRenderer.prototype.rangeBand = function (rangeBandSettings, group, index) {
            ...
            helper_1.drawPath(this.sparkline, pathOption, group);
        };
        SparklineRenderer.prototype.renderLine = function (points, args) {
            ...
            var pathOption = new helper_1.PathOption(spark.element.id + '_sparkline_line', 'transparent', args.lineWidth, color, spark.opacity);
            var d = '';
            ...
            pathOption.d = d;
            helper_1.drawPath(this.sparkline, pathOption, g);
        ...
        SparklineRenderer.prototype.renderPie = function (points, args) {
            ...
            var low;
            var high;
            var locations = ej2_base_1.extend([], [], points);
            ...
            for (var i = 0, stDeg = 90, edDeg = void 0, flag = void 0; i < points.length; i++) {
                ...
                if (!pointArgs.cancel) {
                    var element = helper_1.drawPath(this.sparkline, pathOption, group);
        ...
        SparklineRenderer.prototype.renderArea = function (points, args) {
            ...
            var pathOption = new helper_1.PathOption(spark.element.id + '_sparkline_area', args.fill, 0, 'transparent', spark.opacity);
            var d = '';
            var str = '';
            for (var i = 0, len = points.length; i < len; i++) {
                if (i !== 0) {
            ...
            pathOption.d = d;
            helper_1.drawPath(this.sparkline, pathOption, group);
            pathOption = new helper_1.PathOption(spark.element.id + '_sparkline_area_str', 'transparent', args.border.width, args.border.color, spark.opacity, '', str);
            helper_1.drawPath(this.sparkline, pathOption, group);
            this.sparkline.svgObject.appendChild(group);
        };
        SparklineRenderer.prototype.renderColumn = function (points, args) {
            ...
            var rectOptions = new helper_1.RectOption(id, '', args.border, spark.opacity, null);
            ...
            var _loop_1 = function (i) {
                ...
                rectOptions.rect = new helper_1.Rect(temp.x, temp.y, temp.width, temp.height);
                ...
                this_1.sparkline.trigger('pointRendering', pointArgs, function () {
                    ...
                    temp.location.x = temp.x + (temp.width / 2);
                    if (!pointArgs.cancel) {
                        var element = helper_1.drawRectangle(spark, rectOptions, group);
                        element.setAttribute('aria-label', spark.dataSource[i][spark.xName] + ' : ' + points[i].yVal);
        ...
        SparklineRenderer.prototype.renderWinLoss = function (points, args) {
            ...
            var options = new helper_1.RectOption(id, '', args.border, spark.opacity, null);
            var temp;
            var len = points.length;
            var paletteLength = spark.palette.length;
            for (var i = 0; i < len; i++) {
                ...
                options.rect = new helper_1.Rect(temp.x, temp.y, temp.width, temp.height);
                temp.location.x = temp.x + (temp.width / 2);
                ...
                if (!pointArgs.cancel) {
                    var element = helper_1.drawRectangle(spark, options, group);
                    element.setAttribute('aria-label', spark.dataSource[i][spark.xName] + ' : ' + points[i].yVal);
                }
            }
            this.sparkline.svgObject.appendChild(group);
        ...
        SparklineRenderer.prototype.renderMarker = function (points) {
            ...
            var option = new helper_1.CircleOption('', marker.fill, marker.border, marker.opacity, 0, 0, marker.size / 2, '');
            var highPos;
            var lowPos;
            ...
            var _loop_2 = function (i, length_1) {
                ...
                this_2.sparkline.trigger('markerRendering', markerArgs, function () {
                    if (render && !markerArgs.cancel) {
                        ...
                        var element = helper_1.drawCircle(spark, option, group);
                        element.setAttribute('aria-label', spark.dataSource[i][spark.xName] + ' : ' + points[i].yVal);
                        group.appendChild(element);
                    }
                });
        ...
        SparklineRenderer.prototype.renderLabel = function (points) {
            ...
            var textId = id + 'text_';
            var rectId = id + 'rect_';
            var option = new helper_2.TextOption('', 0, 0, 'middle', '', 'middle');
            ...
            var size = helper_2.measureText('sparkline_measure_text', labelStyle);
            var rectOptions = new helper_1.RectOption('', dataLabel.fill, dataLabel.border, dataLabel.opacity, null);
            var edgeLabelOption;
            ...
            var _loop_3 = function (i, length_2) {
                ...
                this_3.sparkline.trigger('dataLabelRendering', labelArgs, function () {
                    size = helper_2.measureText(labelArgs.text, labelStyle);
                    ...
                    if (renderLabel && !labelArgs.cancel && edgeLabelOption.render) {
                        ...
                        rectOptions.rect = new helper_1.Rect(option.x - ((size.width / 2) + padding), (option.y - padding - (size.height / 1.75)), size.width + (padding * 2), size.height + (padding * 2));
                        g = _this.sparkline.renderer.createGroup({ id: id + 'g' + i });
                        helper_1.drawRectangle(spark, rectOptions, g);
                        helper_2.renderTextElement(option, labelStyle, labelArgs.color, g);
                        group.appendChild(g);
                    }
                });
            };
        ...
        SparklineRenderer.prototype.getInterval = function (data, x) {
            var interval = 1;
            var x1 = data[0][x];
            var x2 = ej2_base_1.isNullOrUndefined(data[1]) ? undefined : data[1][x];
            if (!ej2_base_1.isNullOrUndefined(x1) && !ej2_base_1.isNullOrUndefined(x2)) {
                var temp = ej2_base_1.extend([], data);
                var validData_1 = [];
                temp.forEach(function (value) {
                    if (!ej2_base_1.isNullOrUndefined(value[x])) {
                        validData_1.push(value);
                    }
                });
                validData_1.sort(function (a, b) {
                    if (ej2_base_1.isNullOrUndefined(a[x]) || ej2_base_1.isNullOrUndefined(b[x])) {
                        return 0;
                    }
                    return a[x] - b[x];
                });
        ...
        SparklineRenderer.prototype.findRanges = function (data) {
            ...
            if (model.type !== 'Pie') {
                ...
                maxX = ej2_base_1.isNullOrUndefined(axis.maxX) ? maxX : axis.maxX;
                minX = ej2_base_1.isNullOrUndefined(axis.minX) ? minX : axis.minX;
                max = ej2_base_1.isNullOrUndefined(axis.maxY) ? max : axis.maxY;
                min = ej2_base_1.isNullOrUndefined(axis.minY) ? min : axis.minY;
            ...
            for (var i = 0; i < maxPointsLength; i++) {
                if (ej2_base_1.isNullOrUndefined(data[i][model.xName]) && ej2_base_1.isNullOrUndefined(data[i][model.yName]) && ((data[i][model.yName]) !== 0)
                    && isNumericArray) {
                    x = i;
                    y = data[i];
                }
                else if (ej2_base_1.isNullOrUndefined(data[i][model.xName])) {
                    x = i;
                ...
                if (ej2_base_1.isNullOrUndefined(x) || ej2_base_1.isNullOrUndefined(y)) {
                    continue;
                }
